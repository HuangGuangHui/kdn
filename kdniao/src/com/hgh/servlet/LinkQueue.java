...
public class LinkQueue {
    ...
    private static LinkQueue lq=new LinkQueue();
    ...
    //判空
      ...
      //插入
      public synchronized boolean add(Receive e){
          if(empty()){    //如果队列为空
              front = new Node(e,null);//只有一个节点，front、rear都指向该节点
              rear = front;
          }else{
              Node<Receive> newNode = new Node<Receive>(e, null);
      ...
      //返回队首元素，但不删除
      public synchronized Node<Receive> peek(){
          if(empty()){
      ...
      //出队
      public synchronized Node<Receive> poll(){
          if(empty()){
              throw new RuntimeException("空队列异常！");
          }else{
              Node<Receive> value = front; //得到队列头元素
