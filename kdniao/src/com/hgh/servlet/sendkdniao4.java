...
import com.hgh.comman.commanConnet4;
...
public class sendkdniao4 extends HttpServlet{
	  			...
	  			if(item.isFormField()){
	  				...
	  				if(item.getName()!=null&&!"".equals(item.getName())){
	  					...
	  					for (int i = 1; i < list.size(); i++) {
							...
							String result;
							try {
								result = new commanConnet4().postForm("http://api.kdniao.cc/api/dist", rjson);
...
//	  						new commonDao().insertSenderr(ls);
	  					...
	  					log.info("t4共耗时"+(de.getTime()-ds.getTime())+"毫秒");
	  					log.info("t4共导入"+(list.size()-1)+"条数据,进入接口成功"+num+"条，失败"+(list.size()-num-1)+"条");
	  					file.delete();
	  				}else {
	  					log.error("错误");
					}
